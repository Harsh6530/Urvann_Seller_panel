...
const ReviewSectionScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await axios.get(`http://192.168.137.1:5001/api/reviews/${sellerName}`);
  ...
  const saveData = async () => {
    try {
      ...
      const response = await axios.put(`http://192.168.137.1:5001/api/reviews/${reviewId}`, updatedReviewData);
