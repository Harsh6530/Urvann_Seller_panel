...
const ReviewSectionScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await axios.get(`http://10.5.16.226:5001/api/reviews/${sellerName}`);
  ...
  const saveData = async () => {
    try {
      ...
      const response = await axios.put(`http://10.5.16.226:5001/api/reviews/${reviewId}`, updatedReviewData);
