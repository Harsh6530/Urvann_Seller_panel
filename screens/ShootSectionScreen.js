...
const ShootSectionScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`http://192.168.137.1:5001/api/products/${sellerName}`);
  ...
  const saveData = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://192.168.137.1:5001/api/products/${productId}`, formData);
